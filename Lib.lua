...
local api = {}
local function ApiMakeRequest(TOKEN, method, request_body)
  local response = {}
...
function api.getMe(TOKEN)
  ...
  return ApiMakeRequest(TOKEN, 'getMe', {})

end

function api.getUpdates(TOKEN, offset)
	  
  return ApiMakeRequest(TOKEN, 'getUpdates', {
    timeout = '20',
...
function api.unbanChatMember(TOKEN, chat_id, user_id)
  ...
  return ApiMakeRequest(TOKEN, 'unbanChatMember', {
...
function api.kickChatMember(TOKEN, chat_id, user_id)
  ...
  return ApiMakeRequest(TOKEN, 'kickChatMember', {
    chat_id = tostring(chat_id),
    user_id = tostring(user_id)
  })
...
function api.getChat(TOKEN, chat_id)
	...
	return ApiMakeRequest(TOKEN, 'getChat', {
    chat_id = tostring(chat_id)
  })
...
function api.getChatAdministrators(TOKEN, chat_id)
	 ...
	 return ApiMakeRequest(TOKEN, 'getChatAdministrators', {
    ['chat_id'] = tostring(chat_id)
   })
	
end

function api.deleteMessages(TOKEN, chat_id, msg_id)
	
  return ApiMakeRequest(TOKEN, 'deleteMessage', {
    chat_id = tostring(chat_id),
...
function api.getChatMembersCount(TOKEN, chat_id)
	...
	return ApiMakeRequest(TOKEN, 'getChatMembersCount', {
    chat_id = tostring(chat_id)
  })
...
function api.getChatMember(TOKEN, chat_id, user_id)
	...
	return ApiMakeRequest(TOKEN, 'getChatMember', {
    chat_id = tostring(chat_id),
    user_id = tostring(user_id)
  })
...
function api.leaveChat(TOKEN, chat_id)
	...
	return ApiMakeRequest(TOKEN, 'leaveChat', {
    chat_id = tostring(chat_id)
  })
...
function api.sendMessage(TOKEN, chat_id, Text, parse_mode, reply_markup, reply_to_message_id, link_preview)	
  return ApiMakeRequest(TOKEN, 'sendMessage', {
    ['chat_id'] = tostring(chat_id),
    ['text'] = tostring(Text .. Ex),
    ['reply_to_message_id'] = tostring(reply_to_message_id or 0),
    ['parse_mode'] = tostring(parse_mode),
    ['reply_markup'] = tostring(JSON.encode(reply_markup) or {}),
    ['link_preview'] = tostring(link_preview or true),
    ['disable_notification'] = tostring(disable_notification or false)
  })

end


function api.editMessageText(TOKEN, chat_id, message_id, text, parse_mode, keyboard, DP)
  if parse_mode then
  ...
  return ApiMakeRequest(TOKEN, 'editMessageText', {
    ['chat_id'] = tostring(chat_id),
    ['text'] = tostring(text .. Ex),
    ['message_id'] = tostring(message_id),
    ['parse_mode'] = tostring(parse_mode),
    ['reply_markup'] = JSON.encode(keyboard or {}),
    ['link_preview'] = tostring(DP or true)
  })
...
function api.editMarkup(TOKEN, chat_id, message_id, reply_markup)
	...
	return ApiMakeRequest(TOKEN, 'editMessageReplyMarkup', {
    ['message_id'] = tostring(inline_message_id),
    ['chat_id'] = tostring(chat_id),
...
function api.answerCallbackQuery(TOKEN, callback_query_id, text, show_alert, cache_time)
    ...
    return ApiMakeRequest(TOKEN, 'answerCallbackQuery', {
      ['callback_query_id'] = tostring(callback_query_id),
      ['text'] = tostring(text),
      ['show_alert'] = tostring(show_alert or false),
      ['cache_time'] = tostring(cache_time)
    })	
...
function api.sendChatAction(TOKEN, chat_id, action)
 -- Support actions are typing, upload_photo, record_video, upload_video, record_audio, upload_audio, upload_document, find_location
	return ApiMakeRequest(TOKEN, 'sendChatAction', {
      ['chat_id'] = tostring(chat_id),
...
function api.sendLocation(TOKEN, chat_id, latitude, longitude, reply_to_message_id)
	...
	return ApiMakeRequest(TOKEN, 'sendLocation', {
      ['chat_id'] = tostring(chat_id),
...
function api.forwardMessage(TOKEN, chat_id, from_chat_id, message_id)
  ...
  return ApiMakeRequest(TOKEN, 'forwardMessage', {
      ['chat_id'] = tostring(chat_id),
      ['from_chat_id'] = tostring(from_chat_id),
      ['message_id'] = tostring(message_id)
    })
...
function api.getFile(TOKEN, file_id)
	...
	return ApiMakeRequest(TOKEN, 'getFile', {
      ['file_id'] = file_id
    })
...
function api.answerInlineQuery(TOKEN, inline_query_id, results, cache_time, is_personal, switch_pm_text, switch_pm_parameter)
	...
	return ApiMakeRequest(TOKEN, 'answerInlineQuery', {
      ['inline_query_id'] = tostring(inline_query_id),
      ['results'] = results,
      ['cache_time'] = tostring(cache_time or 'false'),
      ['is_personal'] = tostring(is_personal or 'true'),
      ['switch_pm_text'] = tostring(switch_pm_text or ''),
      ['switch_pm_parameter'] = tostring(switch_pm_parameter or '')
    })
...
function api.sendPhotoId(TOKEN, chat_id, file_id, reply_to_message_id, caption)
	...
	return ApiMakeRequest(TOKEN, 'sendPhoto', {
      ['chat_id'] = tostring(chat_id),
      ['photo'] = file_id,
      ['reply_to_message_id'] = tostring(reply_to_message_id or 0),
...
function api.sendVideoId(TOKEN, chat_id, file_id, reply_to_message_id, caption)
	...
	return ApiMakeRequest(TOKEN, 'sendVideo', {
      ['chat_id'] = tostring(chat_id),
      ['video'] = file_id,
      ['reply_to_message_id'] = tostring(reply_to_message_id or 0),
      ['caption'] = (caption or '')
...
function api.sendDocumentId(TOKEN, chat_id, file_id, reply_to_message_id, caption)
		...
		return ApiMakeRequest(TOKEN, 'sendDocument', {
      ['chat_id'] = tostring(chat_id),
      ['document'] = file_id,
      ['reply_to_message_id'] = tostring(reply_to_message_id or 0),
      ['caption'] = (caption or '')
...
function api.sendPhoto(TOKEN, chat_id, photo, caption, reply_to_message_id)
	...
	if caption then
		curl_command = curl_command .. ' -F "caption=' .. caption.. Ex .. '"'
	end
